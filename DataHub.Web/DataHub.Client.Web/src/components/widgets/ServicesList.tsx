...
const ServicesList = observer(() => {
  ...
  const services = offerings.filter(o => o.itemCategoryType === 1);
    ...
    <LoadingPlaceholder isLoading={isParticipatingLoading}>
      ...
      <Row xs={2} className="mt-2">
        <Col>
          <RequireUser withPermissions={['ManageMyOrganization']}>
            <Button variant="primary" onClick={handleShowUnsubscribe} disabled={selectedService.offeringId === 0}>Remove Service</Button>
          </RequireUser>
        </Col>
      ...
      <Modal centered size='lg' show={showModal} onHide={handleClose} >
        <Modal.Header closeButton>
          <Modal.Title>Active Services</Modal.Title>
        </Modal.Header>
        <Modal.Body>
